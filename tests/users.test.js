...
describe("creating a user", () => {
  ...
  test("fails with proper status code and message if password length is less than 3", async () => {
    ...
    expect(response.body.error).toContain(
      "password must have a minimum length of 3"
    );
...
describe("user login", () => {
  test("is successfull with valid credentials", async () => {
  ...
  test("fails with proper status code and message if credentials is invalid", async () => {
    const credentials = { username: "fingy", password: "1234" };
    const response = await api
      .post("/api/login")
      .send(credentials)
      .expect(400)
